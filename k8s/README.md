...
| `base/database-migration-job.yaml` | Argo CD Sync hook that runs Alembic before gateway workloads |
...
| `base/gateway.yaml` | `CALLBACK_ALLOWED_HOSTS` | The comma-separated n8n hosts that may receive completion callbacks |
...
Argo CD runs `alembic upgrade head` at sync wave 1 and starts the gateway and
reconciler at wave 2. The database itself must already exist; the hook owns
schema migration, not PostgreSQL database provisioning.
